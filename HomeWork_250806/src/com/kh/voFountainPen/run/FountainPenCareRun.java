...
public class FountainPenCareRun {
	...
	public static void main(String[] args) {
		...
		myPen.manufacturer = sc.nextLine();
		System.out.print("길이는 몇 cm인가요? > ");
		myPen.length = sc.nextInt();
		sc.nextLine();
		System.out.print("펜촉의 재질은 어떤 것인가요? > ");
		myPen.nibMaterial = sc.nextLine();
		System.out.print("뚜껑은 어떻게 닫나요? > ");
		myPen.capType = sc.nextLine();
		System.out.print("잉크는 어떻게 충전하나요? > ");
		myPen.inkFillingSystem = sc.nextLine();
		...
		while(!exit) {
			...
			System.out.println(myPen.manufacturer + "에서 만들었어요");
			System.out.println("펜의 길이는 " + myPen.length + "cm 입니다.");
			System.out.println("펜촉은 " + myPen.nibMaterial + "(으)로 만들었어요.");
			System.out.println("뚜껑을 닫는 방식은 " + myPen.capType + "입니다.");
			System.out.println("잉크를 충전하는 방법은 " + myPen.inkFillingSystem + "을/를 사용하게 되어있어요.");
			System.out.println("------------------------------");
			System.out.println();
			
			System.out.println("만년필을 가지고 무엇을 해볼까요? > ");
			
			if(myPen.inkLevel < 15) {
