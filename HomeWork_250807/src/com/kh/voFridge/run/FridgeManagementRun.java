...
public class FridgeManagementRun {
	...
	public static void main(String[] args) {
		...
		myFridge.setBrand(sc.nextLine());
		
		System.out.print("냉장고의 최대 용량은 얼마인가요? (L 단위) > ");
		myFridge.setCapacity(sc.nextInt());
		
		System.out.print("선반이 몇 개 있나요? > ");
		myFridge.setNumberOfShelves(sc.nextInt());
		
		System.out.print("소비 전력은 얼마인가요? (kW) > ");
		myFridge.setPowerConsumption(sc.nextInt());
		
		System.out.print("지금 냉장고는 얼마나 차있나요? (1 ~ 100) > ");
		myFridge.setCurrentStorage(sc.nextInt());
		...
		while(!exit) {
			...
			System.out.println("나의 냉장고는 " + myFridge.getBrand() + "에서 만들었어요.");
			System.out.println("최대 "+ myFridge.getCapacity() + "L의 공간에 보관할 수 있어요!");
			System.out.println("선반은 " + myFridge.getNumberOfShelves() + "개 있어요.");
			System.out.println("소비 전력은 " + myFridge.getPowerConsumption() + "kW 입니다.");
			System.out.println("지금 냉장고는 " + myFridge.getCurrentStorage() + "% 정도 음식이 들어있어요!");
			System.out.println("~~~~~~~~~~ 아 시원한 나의 냉장고 ~~~~~~~~~~");
			System.out.println();
			
			System.out.println("냉장고에 음식을 보관할까요? > ");
			
			if(myFridge.getCurrentStorage() > 70) {
